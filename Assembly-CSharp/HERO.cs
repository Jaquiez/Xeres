...
public class HERO : Photon.MonoBehaviour
{
    ...
    private void dash(float horizontal, float vertical)
    {
        ...
        if (((this.dashTime <= 0f) && (this.currentGas > 0f)) && !this.isMounted)
        {
            ...
            if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
            {
                UnityEngine.Object.Instantiate(Resources.Load((string)handler.XeresPropeties[XeresProperty.gasBurst]), base.transform.position, base.transform.rotation);
                //UnityEngine.Object.Instantiate(Resources.Load("FX/boost_smoke"), base.transform.position, base.transform.rotation);
            }
            else
            {
                PhotonNetwork.Instantiate((string)handler.XeresPropeties[XeresProperty.gasBurst], base.transform.position, base.transform.rotation, 0);
                PhotonNetwork.Instantiate((string)handler.XeresPropeties[XeresProperty.gasBurst], base.transform.position, base.transform.rotation, 0);
            }
            this.dashTime = 0.5f;
                    ...
                    if (this.grounded)
                    {
                        ...
                        else if (this.state == HERO_STATE.Idle)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Xeres.UserPrefs.PropertyHandler handler = GameObject.Find("XeresManager").GetComponent<Xeres.UserPrefs.PropertyHandler>();
                                    if (RCextensions.returnBoolFromObject(handler.XeresPropeties[XeresProperty.AHSSIdle]))
                                    {
